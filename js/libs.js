...
class MakeSpoller {
	...
	showSpoller(anim = true) {
		...
		if (this.isSpollerSelect) {
			...
			setTimeout(() => {
				const cselect = CSelect(this.spollerEl)
				if (!cselect.hasAutoHeight) {
					CScroll(cselect.cselectScroll).update()
				}
			}, duration);
		}
	}
...
document.addEventListener('click', e => {
	...
	if (target.closest('[data-cselect-item]') && !document.querySelector('[data-spoller-body].-anim')) {
		...
		const cselect = CSelect(cselectBlock)
		cselect.setActiveValue(target.closest('[data-cselect-item]'), e.ctrlKey)

		// закриття спойлера
		if (!cselect.hasMultiple || cselect.hasMultiple && !e.ctrlKey) {
...
class MakeCScroll {
	constructor(scrollEl) {
		...
		if (this.hasArrows) {
			this.classIconArrow = 'arrow-right-small'
		}
	...
	init() {
		if (!isMobile.any()) {
			...
			this.scrollWrap.addEventListener('scroll', (e) => {
				this.calcPosititonScroller()
				// користувацька функція onScroll
				if (this.userFunction.onScroll) {
					this.userFunction.onScroll(e)
				}
			})
			this.scrollWrap.addEventListener('scrollend', this.scrollend)
			this.calcPosititonScroller()
	...
	createStructure() {
		...
		const createTemplateScrollBar = (direction) => {
			...
			const templateScrollerStart = '<div data-scrollbar-scroller>'
			const templateScrollerEnd = '</div>'

			let templateScrollerDecor = ''
			if (this.scrollerDecor) {
				for (let index = 0; index < this.scrollerDecor; index++) {
					templateScrollerDecor += '<span></span>'
				}
			}

			const templateScroller = templateScrollerStart + templateScrollerDecor + templateScrollerEnd
			...
			if (this.hasArrows) {
				for (let index = 0; index < templateArrows.length; index++) {
					let templateIcon = ''
					if (this.classIconArrow) {
						templateIcon = `<span class="-icon-${this.classIconArrow}"></span>`
					}
					templateArrows[index] = `<div data-scrollbar-arrow="${index ? 'down' : 'up'}">${templateIcon}</div>`
				}
			}
	...
	calcSizeScroller() {
		...
		this.scrollEl.classList.remove('-has-disable-scrollbar')
		for (const direction in this.scrollBars) {
		...
		for (const direction in this.scrollBars) {
			...
			const sizeValue = Math.round(sizeScrollBarBody * (sizeScrollEl / (sizeScrollContent - sizePadding)))
		...
		if (scrollBarDisabled > 0) {
			this.scrollEl.classList.add('-has-disable-scrollbar')
	...
	calcPosititonScroller = (e) => {
		if (!this.isMovedScroller()) {
			for (const direction in this.scrollBars) {
				...
				scrollBarScroller.style[propertiesAbsolutePoz] = Math.round((sizeScrollBarBody - sizeScroller) * +(valueScrolled / (sizeScrollContent - sizeScrollEl)).toFixed(2)) + 'px'
			}
		}
	...
	sizesElements(direction, indexProperty, paramCoordsMouse = {}) {
		...
		const obj = {
			...
			sizeScroller: scrollBarScroller.getBoundingClientRect()[propertySizeScroller], // розмір бігунка
		...
		if (coordsMouse) {
			obj.areaScrollBarBody = coordsMouse - obj.pozScrollBarBody // область між позицією курсору і початком тіла scrollbar
		}
	...
	scrollend = () => {
		...
		let arrowPress
		for (const direction in this.scrollBars) {
			if (this.scrollBars[direction].el.querySelector('.-press_long')) {
				arrowPress = this.scrollBars[direction].el.querySelector('.-press_long')
			}
		}

		// продоження виконання, якщо є тривале натиснення
		if (arrowPress) {
			this.longPressArrow(arrowPress)
		}
	}
...
function animateMarquee(el, duration) {
    const innerEl = el.querySelector('.-merquee-inner');
...
if (document.querySelector('.partners__row')) {
    const marquee1 = document.querySelector('.partners__row');
    animateMarquee(marquee1, 60000);
}
SmoothScroll({
    animationTime: 500,
    stepSize: 100,
...
if (document.querySelectorAll('[data-spoller]').length > 0) {
    const spollersEl = document.querySelectorAll('[data-spoller]')
    for (let index = 0; index < spollersEl.length; index++) {
        const element = spollersEl[index];
        Spoller(element).init()
    }
}
